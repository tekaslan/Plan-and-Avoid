...
int runEmergencyPlanning(SearchProblem *problem, char * casefolder, char * configfile) {
    ...
    if (problem->solver == SEARCH) {
        ...
        perror("Incorrect solver type. Options -> 3: Landing site selection + Search with ground and airspace risks.\n");
        return EXIT_FAILURE;
    }
}
...
double airspaceRisk(SearchProblem *problem, struct DubinsPath *dubins, char *filename, int type){   
    ...
    if ((uint32_t) numSamples > problem->dynTraffic->nsteps){
        free(coordinates);
        return NAN;
    } 
    ...
    for (int i = 0; i < numSamples; i++){
        ...
        for (uint32_t aircraftID = 0; aircraftID < problem->dynTraffic->nflights; aircraftID++){
            ...
            if (filename && (J > 0)){
                ...
                fprintf(file_conflict, "%d, %.6f, %.6f, %.2f, %.2f, %.6f, %.6f, %.2f, %.2f, %d, %.6f\n",\
