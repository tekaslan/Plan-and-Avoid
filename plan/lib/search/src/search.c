...
int runSearch(SearchProblem * problem){
    ...
    while (!pqIsEmpty(problem->pq) && counter <= problem->maxIter){  
        ...
        // Pop a state -> Returns the top priority state and removes it from the heap
...
/*
    ...
    Ref. Eq. 52 in 
    Tekaslan, H.E.; Atkins, E.M. Gradient-Guided Search for Autonomous Contingency Landing Planning. Drones 2025, 9, 642. https://doi.org/10.3390/drones9090642
*/
...
/*
    Estimates the maximum conflict density ahead in a forward cone
*/
double maxDensityAhead(struct Pos *state, SearchProblem *problem) {
    ...
    int n_rays   = 5;          // Horizontal rays in cone
    int n_points = 1;          // Points along each ray
    int n_level  = 3;          // Vertical levels

    double cone_angle = 60.0;  // total cone spread [deg]
    double stepLength = 3 * problem->lmin * FT_2_NM;  // [NM]
    ...
    for (int i = 0; i < n_rays; i++) {
        ...
        for (int j = 1; j <= n_points; j++) {
            ...
            // Across different glide angles
            for (int k = 0; k < n_level; k++) {

                double gamma;

                if (n_level == 1) {
                    gamma = problem->gammaBG;
                } else {
                    gamma = problem->gammaBG
                          + k * (10.0 - problem->gammaBG) / (n_level - 1);
                }

                new_pos.alt = alt - len * NM_2_FT * tan(gamma * DEG_2_RAD);
                new_pos.t = state->t + len / (problem->ac->airspeed * FT_2_NM);

                for (int m = 0; m < problem->dynTraffic->nflights; m++) {

                    double cost = JCPA(&new_pos, m, (int) round(new_pos.t), problem);
    ...
    // return max_density * walt;
    return sum_density / (n_rays * n_points * n_level * problem->dynTraffic->nflights) * walt;
}

/*
    Dynamic airspace closest point of approach cost
...
double dynamicAirspaceCost(struct Node * parent, struct Node * child, SearchProblem *problem){
    ...
    for (int i = 0; i < N; i++){
        ...
        double Jmax = 0;
        for (int aircraftID = 0; aircraftID < problem->dynTraffic->nflights; aircraftID++){
...
double stateCost(Node *node, SearchProblem * problem){
    ...
    // Uncomment to avoid air traffic corridors.
    ...
    // Comment to avoid air traffic corridors. Use the definition above
    double ha = prohibitedArea_cost;   
    
    // Look ahead heuristic
    double lookahead = maxDensityAhead(&node->state, problem);

    // Weighted heuristic cost h(s)
    if (problem->w_gp && node->gp) node->h = 0.1*node->gd1 + 0.1*node->gd2 + 0.05*node->gchi + 0.1*lookahead + 0.15*ha + 0.5*node->gp; // If ground risk is taken into account
    else node->h = 0.1*node->gd1 + 0.2*node->gd2 + 0.1*node->gchi + 0.1*ha + 0.1*lookahead; // If ground risk is NOT taken into account
...
bool goalTest(Node *node, SearchProblem * problem){
    ...
    // Check final turn loss of well-clear (Commented out as conflict during final approach is common due to spatiotemporal bottleneck.)
...
void searchIntegrationCoordinatesWithFixedTimeStep(SearchProblem *problem,
    ...
    cumulativeDistances(problem, distances); // Cumulative distances in ft

    // Precompute groundspeed at each node [kts]
    ...
    // Build cumulative time along path
    ...
    for (int i = 0; i < N - 1; i++) {
        ...
        // Use average groundspeed over the segment for time accumulation
